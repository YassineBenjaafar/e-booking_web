    ...
    <div class="container">
        ...
        @if (session('statusOut'))
            <div class="alert alert-danger alert-dismissible fade show" role="alert">
                <i class="fa fa-unlink " style="margin-right:10px"></i>
                {{ session('statusOut') }}
            </div>
        @endif
                    ...
                    <div class="social-media">
                        <p class="mb-0 d-flex">
                            <a href="#" class="d-flex align-items-center justify-content-center"><span
                                    class="fa fa-facebook-f"><i class="sr-only">Facebook</i></span></a>
                            <a href="#" class="d-flex align-items-center justify-content-center"><span
        ...
        <nav class="navbar navbar-expand-lg navbar-dark ftco_navbar bg-dark ftco-navbar-light fixed-top" id="ftco-navbar"
            ...
            <div class="container">
                <a class="navbar-brand" href="/">
                    <img src="/assets_template/images/logo.png" alt="brand logo" />
                </a>
                ...
                <div class="collapse navbar-collapse" id="ftco-nav">
                    <ul class="navbar-nav ml-auto">
                        <li class="nav-item"><a href="/" class="nav-link font-weight-bold">{{ __('Acceuil') }}</a></li>
                        <li class="nav-item"><a href="/#contact" class="nav-link font-weight-bold ">{{ __('Contact') }}</a>
                        </li>
                        <li class="nav-item"><a href="/bookings"
                                class="nav-link font-weight-bold">{{ __('Bookings') }}</a></li>
                        </li>
                    </ul>
                    <div class="nav-item"><button class="buttonNotif" data-toggle="modal"
                            data-target="#exampleModalLong"><i class="fa fa-bell"></i><span
                                class="badge badge-pill badge-danger count-notif">{{ count(Auth::guard('client')->user()->unreadNotifications) }}</span></button>
                    </div>

                    <div class="nav-item"><a class="nav-link">{{ Auth::guard('client')->user()->fullName     }}
                        </a>
                    </div>
                    <div class="nav-item">
                        <i class="fa fa-trophy topicons" style="color:#2f89fc;font-size:15px;"></i>
                        <span
                            style="margin-left:5px;color:#2f89fc">{{ Auth::guard('client')->user()->employee->points }}</span>
                    </div>
                    <div class="nav-item">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('feedback.send') }}" method="POST">
                            ...
                            <div class="form-group">
                                <label for="commentaire" style="text-align: center">Commentary</label>
                                <textarea class="form-control" name="commentaire" required></textarea>
                                <input type="text" name="notifID" hidden>
                                <input type="text" name="bookingID" hidden>
                            </div>
                    </div>
                    <div class="modal-footer">
                        <button type="submit" class="btn btn-primary" id="feedbackbtn">Send</button>
                    </div>
                </div>
                </form>
            </div>
                    ...
                    <div class="modal-body divmain">
                        <ul>
                            @php
                                ...
                                $ticketNotifs = $user->unreadNotifications->where('type', 'App\Notifications\ReplyTicketNotification')->sortByDesc('created_at');
                                $bookingNotifs = $user->unreadNotifications->where('type', 'App\Notifications\BookingNotification')->sortByDesc('created_at');
                            ...
                            @foreach ($feedbackNotifs as $notification)
                                <li>
                                    @if ($notification->data['booking']['state_id'] == 4)
                                        <!--  expired -->
                                        <div class="a">
                                            <p>We hope you had a good time during your stay, please
                                                please give us your <a href="" data-dismiss="modal"
                                                    data-toggle="modal" data-target="#exampleModal"
                                                    data-booking-id={{ $notification->data['booking']['id'] }}
                                                    data-notif-id={{ $notification->id }}>feedback </a>?</p>
                                        </div>
                                    @endif
                                </li>
                            @endforeach
                            @foreach ($bookingNotifs as $notification)
                                <li>
                                    @if ($notification->data['booking']['state_id'] != 4)
                                        <div class="a">
                                            <p>Your Booking
                                                <b>{{ date('d-m-Y', strtotime($notification->data['booking']['start_date'])) }}</b>
                                                au
                                                <b>{{ date('d-m-Y', strtotime($notification->data['booking']['end_date'])) }}</b>
                                                crée le
                                                <b>{{ date('d-m-Y', strtotime($notification->data['booking']['created_at'])) }}</b>
                                                est
                                                @php
                                                    $state_id = $notification->data['booking']['state_id'];
                                                    $state = App\State::find($state_id);
                                                @endphp
                                                <a
                                                    href="{{ route('notification.read', $notification) }}">{{ $state->name }}</a>.
                                            </p>
                                        </div>
                                    @endif
                                </li>
                            ...
                            @foreach ($ticketNotifs as $notification)
                                <li>
                                    <div class="a">
                                        <p>Response : {{ $notification->data['admin_reply_message'] }}
                                            <a href="{{ route('notification.read', $notification) }}" style="float: right"
        ...
        <div class="container">
            <div class="row">
                <div class="col-md-6 col-lg-3 mb-md-0 mb-4">
                    <h2 class="footer-heading"><a href="#" class="logo">E-Booking</a></h2>
                </div>
                <div class="col-md-6 col-lg-3 mb-md-0 mb-4">
                    ...
                    <ul class="list-unstyled">
                        ...
                        <li><a href="#" class="py-1 d-block">Central views</a></li>
                    </ul>
                </div>
                <div class="col-md-6 col-lg-3 mb-md-0 mb-4">
                    ...
                    <div class="tagcloud">
                        ...
                        <a href="#" class="tag-cloud-link">lodging</a>
                        <a href="#" class="tag-cloud-link">rent</a>
                        <a href="#" class="tag-cloud-link">entities</a>
                ...
                <div class="col-md-6 col-lg-3 mb-md-0 mb-4">
                    ...
                    <form action="#" class="subscribe-form">
                        <div class="form-group d-flex">
                            ...
                            <button type="submit" class="form-control submit rounded-right"><span
                                    class="sr-only">Send</span><i class="fa fa-paper-plane"></i></button>
                        </div>
                    </form>
                    ...
                    <ul class="ftco-footer-social p-0">
                        ...
                        <li class="ftco-animate"><a href="#" data-toggle="tooltip" data-placement="top"
                                title="Facebook"><span class="fa fa-facebook-f"></span></a></li>
                        <li class="ftco-animate"><a href="#" data-toggle="tooltip" data-placement="top"
